...
public class ProbeProcessor implements IProcessor
{
	  ...
	  public ProbeProcessor(MIDSystem ms, JavascriptProbe probe)
	  {
	    set_probe(probe);
	    set_ms(ms);
	    //Get ConfigParameters object - get_probe().getParameters();
	  ...
	  {
		  HashMap parms = this.get_probe().getParameters();
		  //parms.put("powershell_", "");
		  HashMap parmCopy = new HashMap<String, String>(parms);
		  Powershell ps = new Powershell(this.get_probe().getThreadManager(), parmCopy);
		  File f = new File("scripts/PowerShell/Test.ps1");
		 
		  ProcessRunner p = ps.runScript(f);	
